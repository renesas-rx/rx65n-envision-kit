...
#define PSW_SET_ISP  (0x00000000)
#define SECURE_BOOT_AREA_TOP			(0xFFFF0000)
#define FIRMWARE_USER_RESETVECT_ADDR            (SECURE_BOOT_AREA_TOP - 4)
#endif
...
#if SECURE_BOOT
    /* Enable the bus error interrupt to catch accesses to illegal/reserved areas of memory */
    R_BSP_InterruptControl(BSP_INT_SRC_BUS_ERROR, BSP_INT_CMD_INTERRUPT_DISABLE, FIT_NO_PTR);
    set_psw(PSW_SET_ISP);
    {
    	uint32_t addr;
    	addr = *(uint32_t*)FIRMWARE_USER_RESETVECT_ADDR;
