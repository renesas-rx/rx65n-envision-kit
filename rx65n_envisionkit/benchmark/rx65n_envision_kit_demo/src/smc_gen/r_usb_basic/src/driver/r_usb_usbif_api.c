...
#endif /* #if defined(BSP_MCU_RX65N) || defined(BSP_MCU_RX630) || defined(BSP_MCU_RX63T) */
...
usb_err_t    R_USB_Suspend(usb_ctrl_t *p_ctrl)
{
    ...
    if (USB_STS_CONFIGURED == info.status)
    {
        ...
        err = usb_hstd_change_device_state(&utr, (usb_cb_t)&usb_hstd_suspend_complete,
                                           USB_DO_GLOBAL_SUSPEND, p_ctrl->address);
        if (USB_OK == err)
        {
...
#endif /* #if defined(BSP_MCU_RX63N)   || defined(BSP_MCU_RX65N) || */
...
usb_err_t    R_USB_PipeStop(usb_ctrl_t *p_ctrl)
{
    ...
    if (((USB_NULL == p_ctrl) && (USB_HOST == g_usb_usbmode)) ||
            (USB_PIPE0 == p_ctrl->pipe) || (USB_MAXPIPE_NUM < p_ctrl->pipe))
    {
        return USB_ERR_PARA;
    }
