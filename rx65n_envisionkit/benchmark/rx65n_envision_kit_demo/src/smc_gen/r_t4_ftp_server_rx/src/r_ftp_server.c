...
* Return Value  : none
******************************************************************************/
void R_ftp_srv_open(uint8_t *ipaddr)
{
    int16_t loop;
    int32_t ip_work;
    ...
    ip_work = ((uint32_t)ipaddr[0] << 24) + ((uint32_t)ipaddr[1] << 16) + ((uint32_t)ipaddr[2] << 8) + (uint32_t)ipaddr[3];
...
void R_ftp_srv_ip_address_update(uint8_t *ipaddr)
{
    int16_t loop;
	int32_t ip_work;
            ...
            switch (ftp_stat[i].cep_reset_req & 0xf0)
            {
                ...
                case 0x20:
                    ...
                    if ((ftp_stat[i].cep_reset_req & 0x0f) == 0x0f)
                    {
                        ...
                        ip_work = ((uint32_t)tcpudp_env[0].ipaddr[0] << 24) + ((uint32_t)tcpudp_env[0].ipaddr[1] << 16) + ((uint32_t)tcpudp_env[0].ipaddr[2] << 8) + (uint32_t)tcpudp_env[0].ipaddr[3];
