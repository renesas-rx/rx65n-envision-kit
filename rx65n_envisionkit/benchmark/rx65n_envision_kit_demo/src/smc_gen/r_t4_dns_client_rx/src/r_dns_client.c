...
/***********************************************************************************************************************
* Function Name: htons
* Description  : convert from little endian to big endian when host is little
* Arguments    :
* Return Value : data
***********************************************************************************************************************/
...
dns_err_t R_TCPIP_DnsClientRegisterServerAddress(uint8_t *dns_ipaddr1, uint8_t *dns_ipaddr2)
{
    ...
    if ((0 != dns_ipaddr1) && (0 != dns_ipaddr2))
    {
        ...
        memcpy(&dns_ipaddr_sub.dnsaddr1, dns_ipaddr1, 4);
        memcpy(&dns_ipaddr_sub.dnsaddr2, dns_ipaddr2, 4);

        dns_ipaddr_sub.dnsaddr1 = htonl(dns_ipaddr_sub.dnsaddr1);
        dns_ipaddr_sub.dnsaddr2 = htonl(dns_ipaddr_sub.dnsaddr2);
    }
    else if ((0 != dns_ipaddr1) && (0 == dns_ipaddr2))
    {
        dns_mng.dns_select_sub = DNS_SELECT_SECONDARY;           /* selected dns server <- secondary */
        memcpy(&dns_ipaddr_sub.dnsaddr2, dns_ipaddr1, 4);
        dns_ipaddr_sub.dnsaddr2 = htons(dns_ipaddr_sub.dnsaddr2);
    }
