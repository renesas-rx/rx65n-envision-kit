...
//    uint32_t        reg_buff;
    ...
    /* TRANS mode : Software */
    res = R_SDC_SDMEM_ReadSoftwareTrans(drive, &sdhi_Access);
    if ( SDHI_SUCCESS != res )
    {
        return TFAT_RES_ERROR;
    }
...
//    uint32_t        reg_buff;
    ...
    sdhi_Access.lbn        = sector_number;
    sdhi_Access.cnt        = sector_count;
    sdhi_Access.write_mode = SDC_SD_WRITE_OVERWRITE;

    /* TRANS mode : Software */
    res = R_SDC_SDMEM_WriteSoftwareTrans(drive, &sdhi_Access);
    if ( SDHI_SUCCESS != res )
    {
        return TFAT_RES_ERROR;
    }
