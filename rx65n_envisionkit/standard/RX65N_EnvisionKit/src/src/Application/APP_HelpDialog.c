...
static const char * _apHeaderSpecific[][4] = {
  {
    "2D Drawing Engine Help",
    "Enjoy the 2D Drawing Engine demo!",
  },
  {
    "Bouncing Balls Help",
    "Enjoy the balls bouncing demo!",
  },
  {
    "Mandelbrot Help",
...
static const char * _apBody[][2] = {
  {
    "The 2D Drawing Engine demo shows the efficiency of the 2D graphic accelerator embedded in the RX65N MCU working with semi-transparent images and alpha blending.\nThe clouds ‘sprite’ moves smoothly over a fixed image background with hidden area redrawn dynamically as it is uncovered. The RX Logo can be moved by your finger tip using the touch screen.",
    "Using the touch screen you can freely move the RX logo over the fixed background and animated clouds.\nYou can turn off the 2D Drawing Engine by the provided button to show the performance difference with and without the embedded hardware accelerator.\nWith the Home button you can go back to main screen."
  },
  {
    "The Bouncing Ball demo shows the superior calculation performance of the 32-bit RX65N MCU.\nSeveral balls enter the screen at random angles from the top left and fall and bounce under the influence of a simulated gravity model. The RXv2 Core and the integrated Floating Point Unit (FPU) make the calculations an easy task for the RX65N. In addition the the 2D graphic accelerator embedded in the RX65N MCU can be turned on in addition, which results in massive improvements in regards to the overall performance.",
    "You can turn off the 2D Drawing Engine by the provided button to show the performance difference with and without the embedded hardware accelerator.\nWith the Home button you can go back to main screen."
  },
  {
    "A Mandelbrot set image is generated by resolving an algebraic function for a series of complex numbers and is used as an example of calculation intensive activity for any CPU.\nThe RXv2 Core structure includes:\n- Single Precision Floating Point Unit (FPU) adapts pipeline processing to boost throughput\n- DSP functionality achieves single-cycle throughput on 32bit multiply-accumulate operation using two sets of wide-range accumulators\n- Dual-issue five-stage integer pipeline incorporates enhanced DSP/FPU functionality\nAll of this make the Mandelbrot calculation in real time an easy task for the RX65N.\nHowever, if you use the FPU switch to disable FPU functionality, you can see the tremendous need of FPU support, as the real time calculation of the Mandelbrot calculation almost run into problems without FPU support.",
    "Using the touch screen to plot opposite corners of a rectangular area, you can zoom in to the Mandelbrot set image and define the next set of fractal calculations in real time.\nWith the Back button you will go back to the previous shown screen of the Mandelbrot image.\nWith the FPU switch you can disable the FPU support. Note that this would result in massive calculation efforts for the MCU.\nWith the Home button you can go back to main screen."
  },
  {
    "It is possible to update the firmware by inserting a USB memory stick containing suitable firmware image into the USB Host interface of the Envision Kit.\nTo obtain a firmware image to be copied to your blank USB memory stick, please go to www.renesas.com/envision.\nOnce the new firmware image has been flashed into the alternate embedded flash memory bank, you can quickly swap between the two firmware versions using the Bank Swap dialogue. In the default kit shipment you will be able to find a benchmark firmware on the second memory bank.",
    "Insert USB memory stick while pushing user button on the Envision kit within the Bank Swap demo screen to flash new firmware to the RX.\nIn the Bank Swap screen you can swap between the two firmware versions by pressing the Swap button.\nWith the Home button you can go back to main screen. Please ensure that any new firmware you do flash into a bank, supports a swap functionality, otherwise you have no chance to enter the second flash bank anymore, except by using the on chip debugger."
  },
};
