//--------------------------------------------------------------------------
// Project: D/AVE
// File:    dave_base_nios.c (%version: 8 %)
...
static void d1_mstp_set(bool enable, int mstp_char, int mstp_num)
{
    unsigned long *mstp_addr;

    mstp_addr = ((unsigned long *)MSTP_0_BASE) + mstp_char;

    if (true == enable)
    {
        *mstp_addr = (*mstp_addr & (~(1 << mstp_num)));
    }
    else
    {
        *mstp_addr = (*mstp_addr | (1 << mstp_num));
...
static void d1_registerprotectenable()
{
    unsigned short *prcr_addr;

    prcr_addr = (unsigned short *)PRCR_0_BASE;
    *prcr_addr = ((*prcr_addr | PRCR_KEY) & (~PRCR_PRC1));
}
